module sp_pipeline(
  ...
  (* MARK_DEBUG = "true" *) output wire [23:0] aer_packet,       // 24-bit AER output [ch:timestamp]
  (* MARK_DEBUG = "true" *)  output wire        aer_valid,        // AER packet valid flag
  (* MARK_DEBUG = "true" *)  output wire        audio_done       // High when ROM playback is finished
    ...
    // 🛠️ DEBUG PORTS (Probe these in Waveform Viewer)
     wire [15:0] debug_rom;        // Raw ROM PCM output
     wire [15:0] debug_emph;       // Pre-emphasis filter output
     wire [15:0] debug_env0;       // IHC Envelope for Channel 0 (100Hz Band)
     wire [15:0] debug_spike_bus;   // 16-bit binary spike vector

wire clk_125_bufg;
    wire clk;  // Main system clock (125 MHz)
...
ila_0 your_instance_name (
	...
	.probe1(audio_done), // input wire [0:0]  probe1 
	.probe2(aer_packet), // input wire [23:0]  probe2 
	.probe3(aer_valid) // input wire [0:0]  probe3
);
endmodule
